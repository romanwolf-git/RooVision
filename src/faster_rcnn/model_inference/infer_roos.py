"""script to infer on a local image using roboflow or torchvision API"""
...
from faster_rcnn.model_architecture.get_model import load_roboflow_model
from faster_rcnn.data_augmentation.custom_transforms import get_transform
from faster_rcnn.utils.custom_utils import ROOT_DIR, read_yaml
...
def get_img_path_out(img_path):
    img_path_out = img_path.replace('img_upload', 'img_output')
...
def display_and_safe_img(img, img_path_out):
    """
    Save and show a PIL image.
    :param img: PIL image to save and display
    :param img_path_out: path to where to save the file
    """
...
def infer_save_torchvision(model, img_path, confidence, overlap):
    ...
    img_path_out = get_img_path_out(img_path)
    display_and_safe_img(drawn_boxes, img_path_out=img_path_out)
...
def detect_roos_torch(model, img_path, img_path_out, confidence):
    ...
    :param img_path_out: Path to save the img_output image with bounding boxes.
...
if __name__ == "__main__":
    ...
    img_path = os.path.join(ROOT_DIR / 'data' / 'images' / 'train' / 'images' / 'macropus_fuliginosus_065_jpg.rf'
